...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Invalid JSON body" }, { status: 400, headers: privateNoStore });
  }

  if (!body.url) {
    return NextResponse.json({ error: "url is required" }, { status: 400, headers: privateNoStore });
  }

  const result = await runGeoAuditForUser(body.url, apiKey.userId, apiKey.teamId || undefined);

  if (!result.success) {
    return NextResponse.json({ error: result.error }, { status: 422, headers: privateNoStore });
  }
